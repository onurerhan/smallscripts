...
def print_target(target, query_type="ANY", subdomains="tools/subdomain_scanner/names.txt", resolve_list="tools/subdomain_scanner/resolvers.txt", process_count=16,
...
def run(target, query_type="ANY", subdomains="tools/subdomain_scanner/names.txt", resolve_list=False, process_count=8):
    spider_blacklist = {}
    result_blacklist = {}
    found_domains = {}
    ...
    if resolve_list:
        ...
        if (len(resolve_list) / 16) < process_count:
            sys.stderr.write('Warning: Fewer than 16 resolvers per process, '
...
if __name__ == "__main__":
    ...
    parser.add_option("-s", "--subs", dest="subs", default=os.path.join(base_path, "names.txt"), type="string",
                      help="(optional) A list of subdomains, accepts a single file, or a directory of files. "
                           "default = 'names.txt'")
    ...
    parser.add_option("--type", dest="type", default=False, type="string",
                      help="(optional) Print all reponses for an arbitrary DNS record type "
                           "(CNAME, AAAA, TXT, SOA, MX...)")
    parser.add_option("-c", "--process_count", dest="process_count", default=8, type="int",
